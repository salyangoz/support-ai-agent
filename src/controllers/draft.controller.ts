...
export async function listByTicket(
  ...
  try {
    ...
    const drafts = await draftService.getDraftsByTicketId(tenantId, ticketId);
...
export async function updateStatus(
  ...
  try {
    ...
    const draft = await draftService.updateDraftStatus(tenantId, draftId, status, reviewed_by);
...
export async function send(
  ...
  try {
    ...
    if (draft.status !== 'approved') {
      res.status(400).json({ error: 'Draft must be approved before sending' });
      return;
    }
