...
export async function webhookAuth(
  ...
  try {
    ...
    const tenantProvider = await findProvider(tenant.id, provider) as TenantProvider | null;

    if (!tenantProvider) {
      res.status(400).json({ error: 'Provider not configured for this tenant' });
      return;
    }
    ...
    const adapter = createProvider({ ...tenantProvider.credentials, provider });
